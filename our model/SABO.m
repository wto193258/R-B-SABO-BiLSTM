...
function[Best_score,Best_pos,SABO_curve]=SABO(N,T,lo,hi,m,fitness,num_train_s)
vp_train = evalin('caller', 'vp_train');
vt_train = evalin('caller', 'vt_train');
...
for t=1:T  % algorithm iteration
    ...
    for i=1:N
        ...
        L=X_new_P1;
        fit_new_P1=fitness(L,num_train_s);
        if fit_new_P1<fit(i)
            X(i,:) = X_new_P1;
            fit(i) = fit_new_P1;
        end
